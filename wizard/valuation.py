...
class Propertyvaluation(models.TransientModel):
    ...
    tag_ids = fields.Selection(related='Property_value_id.tag_id', tracking=True, ondelete='cascade')
    ...
    user_entered_price = fields.Float(string='User Entered Price', required=True)
    ...
    def get_per_sqm_price(self):
        return float(self.env['ir.config_parameter'].sudo().get_param('property_valuation.per_sqm_price', default=0))
